...
function define_subproblem_decision_variables!(scuc_subproblem::Model, NT, NG, ND, NC, ND2, NS, NW, config_param)
	# binary variables
	@variable(scuc_subproblem, relaxed_x[1:NG, 1:NT])
	@variable(scuc_subproblem, relaxed_u[1:NG, 1:NT])
	@variable(scuc_subproblem, relaxed_v[1:NG, 1:NT])
	# @variable(scuc_subproblem, relaxed_su₀[1:NG, 1:NT] >= 0)
	# @variable(scuc_subproblem, relaxed_sd₀[1:NG, 1:NT] >= 0)
...
function set_subproblem_objective_economic!(
	...
	relaxed_x = scuc_subproblem[:relaxed_x]
	relaxed_su₀ = scuc_subproblem[:relaxed_su₀]
	relaxed_sd₀ = scuc_subproblem[:relaxed_sd₀]
