...
function add_unit_operation_constraints!(scuc::Model, NT, NG, units, onoffinit)
	...
	for i in 1:NG
		for t in Int64(max(1, Lupmin[i])):NT
			...
			tem_up = @constraint(scuc, sum(u[i, r] for r in LB:t) <= x[i, t], base_name = base_name_con₁)
			push!(units_minuptime_constr, tem_up)
		end

		for t in Int64(max(1, Ldownmin[i])):NT
			base_name_con₂ = "units_min_down_time" * "_" * string(i) * "_" * string(t)
			LB = Int64(max(t - units.min_shutup_time[i, 1] + 1, 1))
			tem_down = @constraint(scuc, sum(v[i, r] for r in LB:t) <= (1 - x[i, t]), base_name = base_name_con₂)
			push!(units_mindowntime_constr, tem_down)
		end
	end
	...
	units_init_shutup_cost_constr = @constraint(scuc, units_initial_upcost, su₀[:, 1] .>= shutupcost .* (x[:, 1] - onoffinit[:, 1]))
	units_init_shutdown_cost_costr = @constraint(scuc, units_initial_dwcost, sd₀[:, 1] .>= shutdowncost .* (onoffinit[:, 1] - x[:, 1]))
	units_shutup_cost_constr = @constraint(scuc, units_upcost[t = 2:NT], su₀[:, t] .>= shutupcost .* u[:, t])
	units_shutdown_cost_constr = @constraint(scuc, units_dwcost[t = 2:NT], sd₀[:, t] .>= shutdowncost .* v[:, t])
